...
class Model(nn.Module):
    ...
    def forward(self, x, s_x=torch.FloatTensor(1, 1, 3, 473, 473).cuda(), s_y=torch.FloatTensor(1, 1, 473, 473).cuda(),
        ...
        if self.training:
            ...
            # aux_probs = 1-probs
#             bg_mask = F.interpolate(y, size=(query_feat.size(2), query_feat.size(3)), mode='bilinear',align_corners=True)
            bg_prototype =  Weighted_GAP(query_feat,bg_mask)
            # aux_proto_bg = Weighted_GAP(query_feat,mask)
            # pri_proto_bg = Weighted_GAP(query_feat,mask)
            # calculate bgmask
            ...
            corr_query_bgmask = F.interpolate(corr_query_bgmask, size=(query_feat.size(2), query_feat.size(3)),
                                            mode='bilinear',
                                            align_corners=True)
            bg, bg_loss = self.decoder(corr_query_bgmask,[bg_prototype,bg_prototype], query_feat,y_.squeeze(1))
