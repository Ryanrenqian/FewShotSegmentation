...
class Model(nn.Module):
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), s_seed=None, y=None):
        ...
        for idx, tmp_bin in enumerate(self.pyramid_bins):
            ...
            supp_feat_bin = self.multinorm(supp_feat_bin)
            query_feat_bin = self.multinorm(query_feat_bin)
    ...
    def multinorm(self,feature: torch.Tensor,K=4,reduce_dim=1536):
        features = torch.split(feature,reduce_dim//K,dim=1)
        output = [feat/torch.norm(feat,2, 1,keepdim=True)  for feat in features]
