...
class Model(nn.Module):
    ...
    def forward(self, x, s_x=torch.FloatTensor(1, 1, 3, 473, 473).cuda(), s_y=torch.FloatTensor(1, 1, 473, 473).cuda(),
        ...
        for i in range(self.shot):
            ...
            origin_proto,pri_proto,aux_proto,_ = self.generate_proto(supp_feat,mask)
            pri_proto_list.append(pri_proto)
            aux_proto_list.append(aux_proto)
            origin_proto_list.append(origin_proto)
        ...
        origin_proto = origin_proto_list[0]
        ...
        if self.shot > 1:
            for i in range(1, len(pri_proto_list)):
                ...
                origin_proto += origin_proto_list[i]
            pri_proto /= len(pri_proto_list)
            aux_proto /= len(pri_proto_list)
            origin_proto /= len(pri_proto_list)
        _, fg_loss_aux = self.decoder(corr_query_mask,[origin_proto,origin_proto], query_feat,y.long())
        out, fgloss = self.decoder(corr_query_mask,[pri_proto,aux_proto], query_feat,y.long())
        fgloss = 0.5 * fg_loss_aux + 0.5 *fgloss
        ...
        if self.training:
            ...
            bg_mask = F.interpolate(y_.float(), size=(query_feat_4.size(2), query_feat_4.size(3)), mode='bilinear',
                                    align_corners=True)
            _,_,_,probs = self.generate_proto(query_feat_4,bg_mask)
    ...
    def generate_proto(self,feats,mask):
        ...
        return origin_proto,pri_proto,aux_proto,probs
