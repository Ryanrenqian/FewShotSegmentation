...
class Model(nn.Module):
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), s_seed=None, y=None):
        ...
        for i in range(self.shot):
            ...
            pri_proto,aux_proto,probs = self.generate_proto(final_supp_feat_4,mask)
    ...
    def generate_proto(self,feats,mask):
        mask = F.interpolate(mask, size=(feats.size(2), feats.size(3)), mode='bilinear', align_corners=True)
