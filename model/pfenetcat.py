...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.down_supp = nn.Sequential(
            nn.Conv2d(fea_dim+1, reduce_dim, kernel_size=1, padding=0, bias=False),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.5)                   
        )  
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), s_seed=None, y=None):
        ...
        for i in range(self.shot):
            ...
            with torch.no_grad():
                ...
                mask = F.interpolate(mask, size=(supp_feat_4.size(2), supp_feat_4.size(3)), mode='bilinear',
                                     align_corners=True)

                final_supp_list.append(supp_feat_4)

            mask_0 = F.interpolate(mask, size=(supp_feat_3.size(2), supp_feat_3.size(3)), mode='bilinear',
                                     align_corners=True)
            supp_feat = torch.cat([supp_feat_3, supp_feat_2,mask_0 ], 1)
