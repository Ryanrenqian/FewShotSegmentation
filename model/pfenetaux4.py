...
class Model(nn.Module):
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), s_seed=None, y=None):
        ...
        for i in range(self.shot):
            ...
            feat_v = Weighted_GAP(supp_feat, mask)
            pri_proto_list.append(feat_v)
            ...
            total = mask.sum()
            iteration = 0
            while(iteration< self.EM_k):
                current_mask = F.cosine_similarity(supp_feat,feat_v,dim=1).unsqueeze(1)>0.5
                rest_mask = rest_mask - rest_mask *  current_mask
                feat_v = Weighted_GAP(supp_feat, rest_mask)
                pri_proto_list.append(feat_v)
                rest = rest_mask.sum()
                iteration += 1
        ...
        out,out_list = self.decoder(pri_proto_list,query_feat)
    ...
    def decoder(self,prototypes: list, query_feat:torch.Tensor):
        '''
        ...
        for idx, tmp_bin in enumerate(self.pyramid_bins):
            ...
            merge_feat_bin = torch.cat([query_feat_bin, proto_feat_bin], 1)
