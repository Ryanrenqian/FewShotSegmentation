...
class Model(nn.Module):
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), s_seed=None, y=None):
        ...
        corr_query_mask = F.interpolate(y.float().unsqueeze(1), size=(query_feat.size(2), query_feat.size(3)), mode='bilinear',
                                        align_corners=True)
        # print(corr_query_mask.size())
        if self.shot > 1:
            supp_feat = supp_feat_list[0]
            for i in range(1, len(supp_feat_list)):
                supp_feat += supp_feat_list[i]
            supp_feat /= len(supp_feat_list)
        # supp_feat =  supp_feat * F.softmax(supp_feat*supp_feat)
        out_list = []
        pyramid_feat_list = []
